async function fetchClues(acrossAnswers, downAnswers) {
  const resp = await fetch("/api/generate-clues", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ across: acrossAnswers, down: downAnswers })
  });
  if (!resp.ok) {
    const err = await resp.text();
    throw new Error("Clue generation failed: " + err);
  }
  return resp.json(); // { acrossClues: [...], downClues: [...] }
}

// Once the WASM runtime is ready, kick everything off:
Module.onRuntimeInitialized = async () => {
  try {
    // 1) call into your compiled C
    const generate = Module.cwrap("generate", "string", []);
    let grid25 = generate();
    if (!grid25) {
      console.error("crossword generation failed");
      return;
    }
    grid25 = grid25.toUpperCase();

    // 2) slice into across/down answers
    const across = [
      grid25.slice(0,  5),
      grid25.slice(5, 10),
      grid25.slice(10,15),
    ...
    const down = Array.from({ length: 5 }, (_, c) =>
      Array.from({ length: 5 }, (_, r) => grid25[r*5 + c]).join("")
    );

    // 3) hit your server for clues
    const { acrossClues, downClues } = await fetchClues(across, down);

    // 4) now launch your UI
    startApp(grid25, acrossClues, downClues);
...
function startApp(answer, acrossClues, downClues) {
    let currentSquare = 0;
    ...
    for (let i = 0; i < 5; i++) {
        document.getElementById(`acrossclue${i+1}`).textContent = acrossClues[i];
        document.getElementById(`downclue${i+1}`).textContent = downClues[i];
    };
