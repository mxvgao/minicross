Module.onRuntimeInitialized = () => {
  // wrap your C function
  const generate = Module.cwrap("generate", "string", []);

  // call it
  const grid25 = generate();
  if (!grid25) {
    console.error("crossword generation failed");
    return;
  }

  // hand it off to your existing UI bootstrap
  startApp(grid25);
};

function startApp(answer) {
    let currentSquare = 0;
    ...
    const across = [
        ...
        "A cowboy's rope",
        'Chutzpah',
        "A toddler's rocking toy"
    ];
    const down = [
        'Hogwarts caretaker',
        'The US state of potatoes',
        "Choir's platform",
        'Red Sea parter',
        'Like the air near a fire'
    ];
    ...
    renderList('across-list', across);
    renderList('down-list', down);
